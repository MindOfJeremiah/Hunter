# make_plan.py — generates a beautiful shareable beach day plan page.
# Usage:
#   python3 make_plan.py dockweiler --for Kelli --note "just trust me on this one"

import sys
import os
import webbrowser
from beach_day import BEACHES

PRICE_LABEL = {"$": "budget-friendly", "$$": "mid", "$$$": "splurge"}

def generate(beach_key, for_name=None, note=None):
    b = BEACHES[beach_key]
    name_display = for_name if for_name else "today"
    note_html = f'<div class="intention-note">&#x201C;{note}&#x201D;</div>' if note else ""
    fire_pack = '<li class="pack-item" data-id="lighter"><span class="check-box"></span><span>lighter or matches 🔥</span></li>' if b["fire_pit"] else ""
    fire_badge = '<span class="fp-badge">🔥 fire pits allowed</span>' if b["fire_pit"] else ""

    food_cards = ""
    for i, f in enumerate(b["food_nearby"]):
        label = PRICE_LABEL.get(f["price"], f["price"])
        food_cards += f"""
        <div class="food-card" data-id="food{i}" onclick="pickFood(this)">
          <div class="food-top">
            <span class="food-name">{f['name']}</span>
            <span class="price-tag">{label}</span>
          </div>
          <div class="food-meta">{f['type']} &middot; {f['distance']}</div>
          <div class="food-note">{f['note']}</div>
          <div class="pick-label">tap to pick this one</div>
          <div class="picked-label">&#10003; this is the pick</div>
        </div>"""

    spot_items = ""
    for i, s in enumerate(b["spots_nearby"]):
        spot_items += f'<li class="spot-item" data-id="spot{i}" onclick="toggleSpot(this)"><span class="check-box"></span><span>{s}</span></li>'

    vibes = "  &middot;  ".join(b["vibe"])

    html = f"""<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>a day for {name_display}</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,500;0,700;1,400&family=DM+Sans:wght@300;400;500&display=swap" rel="stylesheet">
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}

    :root {{
      --pink:       #E8638C;
      --pink-light: #FFB6D9;
      --pink-pale:  #FFF0F7;
      --rose:       #D4456B;
      --crimson:    #8B1A38;
      --gold:       #D4A843;
      --cream:      #FFF8F0;
      --text:       #2A1220;
      --muted:      #A07080;
    }}

    body {{
      font-family: 'DM Sans', -apple-system, sans-serif;
      background: var(--cream);
      color: var(--text);
      min-height: 100vh;
      overflow-x: hidden;
    }}

    /* ── HERO ── */
    .hero {{
      position: relative;
      min-height: 340px;
      background: linear-gradient(
        160deg,
        #5C1030 0%,
        #8B1A38 18%,
        #C04070 38%,
        #E8638C 58%,
        #F4A0C0 78%,
        #FFD6E8 100%
      );
      overflow: hidden;
      display: flex;
      flex-direction: column;
      justify-content: flex-end;
      padding: 32px 28px 36px;
    }}

    /* Juneteenth star-burst watermark */
    .hero::before {{
      content: '✦';
      position: absolute;
      top: 24px; right: 28px;
      font-size: 64px;
      color: rgba(212,168,67,0.22);
      animation: pulse 4s ease-in-out infinite;
    }}

    @keyframes pulse {{
      0%, 100% {{ opacity: 0.22; transform: scale(1); }}
      50% {{ opacity: 0.38; transform: scale(1.08); }}
    }}

    /* Clouds */
    .clouds {{ position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; }}
    .cloud {{
      position: absolute;
      background: rgba(255,255,255,0.18);
      border-radius: 60px;
      animation: drift linear infinite;
    }}
    .cloud::before, .cloud::after {{
      content: '';
      position: absolute;
      background: inherit;
      border-radius: 50%;
    }}
    .c1 {{ width:120px; height:36px; top:15%; animation-duration:28s; animation-delay:0s; }}
    .c1::before {{ width:56px; height:56px; top:-28px; left:18px; }}
    .c1::after  {{ width:40px; height:40px; top:-20px; left:55px; }}
    .c2 {{ width:80px;  height:26px; top:35%; animation-duration:38s; animation-delay:-12s; opacity:0.7; }}
    .c2::before {{ width:38px; height:38px; top:-20px; left:12px; }}
    .c2::after  {{ width:28px; height:28px; top:-14px; left:38px; }}
    .c3 {{ width:160px; height:44px; top:8%;  animation-duration:50s; animation-delay:-25s; opacity:0.5; }}
    .c3::before {{ width:70px; height:70px; top:-38px; left:22px; }}
    .c3::after  {{ width:50px; height:50px; top:-26px; left:80px; }}
    @keyframes drift {{
      from {{ transform: translateX(-220px); }}
      to   {{ transform: translateX(calc(100vw + 220px)); }}
    }}

    /* Floating petals */
    .petals {{ position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; overflow: hidden; }}
    .petal {{
      position: absolute;
      font-size: 18px;
      animation: fall linear infinite;
      opacity: 0.75;
    }}
    .p1  {{ left:8%;   animation-duration:9s;  animation-delay:0s;   font-size:14px; }}
    .p2  {{ left:22%;  animation-duration:13s; animation-delay:-3s;  font-size:20px; }}
    .p3  {{ left:40%;  animation-duration:11s; animation-delay:-6s;  font-size:12px; }}
    .p4  {{ left:60%;  animation-duration:15s; animation-delay:-1s;  font-size:18px; }}
    .p5  {{ left:75%;  animation-duration:10s; animation-delay:-8s;  font-size:16px; }}
    .p6  {{ left:90%;  animation-duration:14s; animation-delay:-4s;  font-size:14px; }}
    .p7  {{ left:50%;  animation-duration:12s; animation-delay:-9s;  font-size:22px; }}
    .p8  {{ left:32%;  animation-duration:16s; animation-delay:-2s;  font-size:12px; }}
    @keyframes fall {{
      0%   {{ transform: translateY(-30px) rotate(0deg);   opacity: 0; }}
      10%  {{ opacity: 0.75; }}
      90%  {{ opacity: 0.4; }}
      100% {{ transform: translateY(360px) rotate(540deg); opacity: 0; }}
    }}

    .juneteenth-line {{
      font-size: 11px;
      letter-spacing: 0.14em;
      text-transform: uppercase;
      color: var(--gold);
      margin-bottom: 10px;
      font-weight: 500;
    }}
    .for-name {{
      font-family: 'Playfair Display', serif;
      font-style: italic;
      font-size: 15px;
      color: rgba(255,255,255,0.75);
      margin-bottom: 6px;
    }}
    .hero h1 {{
      font-family: 'Playfair Display', serif;
      font-size: clamp(32px, 8vw, 48px);
      font-weight: 700;
      color: #fff;
      line-height: 1.1;
      text-shadow: 0 2px 12px rgba(90,10,40,0.35);
      margin-bottom: 6px;
    }}
    .hero-city {{
      font-size: 14px;
      color: rgba(255,255,255,0.65);
      margin-bottom: 14px;
    }}
    .vibe-chips {{
      display: flex;
      flex-wrap: wrap;
      gap: 6px;
    }}
    .vibe-chip {{
      font-size: 11px;
      padding: 4px 11px;
      border-radius: 99px;
      background: rgba(255,255,255,0.18);
      color: rgba(255,255,255,0.9);
      backdrop-filter: blur(4px);
      border: 1px solid rgba(255,255,255,0.25);
    }}
    .fp-badge {{
      font-size: 11px;
      padding: 4px 11px;
      border-radius: 99px;
      background: rgba(212,168,67,0.28);
      color: var(--gold);
      border: 1px solid rgba(212,168,67,0.4);
    }}

    /* ── BODY ── */
    .body-wrap {{
      padding: 0 20px 60px;
      max-width: 640px;
      margin: 0 auto;
    }}

    /* Intention card */
    .intention {{
      background: linear-gradient(135deg, #F8E8F0, #FFF0F7);
      border-radius: 20px;
      padding: 22px 24px;
      margin: 28px 0 0;
      border: 1px solid rgba(232,99,140,0.15);
      position: relative;
    }}
    .intention::before {{
      content: '🌸';
      position: absolute;
      top: -12px; right: 20px;
      font-size: 24px;
    }}
    .intention-label {{
      font-size: 10px;
      letter-spacing: 0.12em;
      text-transform: uppercase;
      color: var(--pink);
      font-weight: 500;
      margin-bottom: 10px;
    }}
    .intention-note {{
      font-family: 'Playfair Display', serif;
      font-style: italic;
      font-size: 17px;
      color: var(--crimson);
      line-height: 1.55;
    }}
    .beach-blurb {{
      font-size: 15px;
      color: #5A3040;
      line-height: 1.65;
      margin-top: 12px;
    }}

    /* Meet up time */
    .meetup {{
      margin-top: 22px;
      background: #fff;
      border-radius: 16px;
      padding: 18px 20px;
      border: 1px solid #F0D8E8;
      display: flex;
      align-items: center;
      gap: 14px;
    }}
    .meetup-icon {{ font-size: 26px; flex-shrink: 0; }}
    .meetup-label {{
      font-size: 11px;
      text-transform: uppercase;
      letter-spacing: 0.1em;
      color: var(--muted);
      margin-bottom: 4px;
    }}
    .meetup-time {{
      font-size: 22px;
      font-family: 'Playfair Display', serif;
      font-weight: 700;
      color: var(--crimson);
      cursor: pointer;
      border: none;
      background: none;
      padding: 0;
      outline: none;
      width: 100%;
    }}
    .meetup-time::placeholder {{ color: #D4A0B8; }}

    /* Section headers */
    .section {{ margin-top: 32px; }}
    .section-header {{
      display: flex;
      align-items: center;
      gap: 10px;
      margin-bottom: 14px;
    }}
    .section-icon {{ font-size: 20px; }}
    .section-title {{
      font-size: 11px;
      letter-spacing: 0.12em;
      text-transform: uppercase;
      color: var(--muted);
      font-weight: 500;
    }}
    .section-line {{
      flex: 1;
      height: 1px;
      background: linear-gradient(90deg, #F0D0E0, transparent);
    }}

    /* Packing list */
    .pack-list {{ list-style: none; }}
    .pack-item {{
      display: flex;
      align-items: center;
      gap: 12px;
      padding: 12px 16px;
      background: #fff;
      border-radius: 12px;
      margin-bottom: 8px;
      border: 1px solid #F5E5EE;
      cursor: pointer;
      transition: all 0.2s;
      font-size: 14px;
      color: #4A2030;
      user-select: none;
    }}
    .pack-item.checked {{
      background: #FFF0F7;
      border-color: #F0C8DC;
    }}
    .pack-item.checked span:last-child {{ text-decoration: line-through; color: var(--pink-light); }}
    .check-box {{
      width: 22px; height: 22px;
      border-radius: 50%;
      border: 2px solid #E8B0CC;
      flex-shrink: 0;
      display: flex;
      align-items: center;
      justify-content: center;
      transition: all 0.2s;
      font-size: 13px;
    }}
    .pack-item.checked .check-box {{
      background: var(--pink);
      border-color: var(--pink);
      color: #fff;
    }}
    .pack-item.checked .check-box::after {{ content: '✓'; }}

    /* Food cards */
    .food-card {{
      background: #fff;
      border-radius: 16px;
      padding: 16px 18px;
      margin-bottom: 12px;
      border: 2px solid #F5E5EE;
      cursor: pointer;
      transition: all 0.25s;
      position: relative;
      overflow: hidden;
    }}
    .food-card:active {{ transform: scale(0.98); }}
    .food-card.picked {{
      border-color: var(--pink);
      background: linear-gradient(135deg, #FFF5F8, #FFE8F2);
    }}
    .food-top {{
      display: flex;
      justify-content: space-between;
      align-items: flex-start;
      margin-bottom: 4px;
    }}
    .food-name {{ font-weight: 600; font-size: 15px; color: #2A1220; flex: 1; }}
    .price-tag {{
      font-size: 11px;
      padding: 3px 9px;
      border-radius: 99px;
      background: #F5EAF0;
      color: var(--muted);
      flex-shrink: 0;
      margin-left: 8px;
    }}
    .food-card.picked .price-tag {{ background: rgba(232,99,140,0.15); color: var(--rose); }}
    .food-meta {{ font-size: 12px; color: #B090A0; margin-bottom: 5px; }}
    .food-note {{ font-size: 13px; color: #6A4050; line-height: 1.4; }}
    .pick-label {{
      font-size: 11px;
      color: #D0A0B8;
      margin-top: 10px;
      text-align: right;
    }}
    .picked-label {{
      font-size: 11px;
      color: var(--pink);
      font-weight: 500;
      margin-top: 10px;
      text-align: right;
      display: none;
    }}
    .food-card.picked .pick-label {{ display: none; }}
    .food-card.picked .picked-label {{ display: block; }}

    /* Spots */
    .spot-item {{
      display: flex;
      align-items: flex-start;
      gap: 12px;
      padding: 13px 16px;
      background: #fff;
      border-radius: 12px;
      margin-bottom: 8px;
      border: 1px solid #F5E5EE;
      cursor: pointer;
      transition: all 0.2s;
      font-size: 14px;
      color: #4A2030;
      line-height: 1.45;
      user-select: none;
    }}
    .spot-item.done {{
      background: #FFF0F7;
      border-color: #F0C8DC;
    }}
    .spot-item.done span:last-child {{ text-decoration: line-through; color: var(--pink-light); }}
    .spot-item .check-box {{ margin-top: 1px; flex-shrink: 0; }}
    .spot-item.done .check-box {{
      background: var(--pink);
      border-color: var(--pink);
      color: #fff;
    }}
    .spot-item.done .check-box::after {{ content: '✓'; }}

    /* Parking */
    .parking-card {{
      background: #fff;
      border-radius: 16px;
      padding: 16px 18px;
      border: 1px solid #F5E5EE;
      font-size: 14px;
      color: #5A3040;
      line-height: 1.5;
    }}

    /* Notes */
    .notes-area {{
      width: 100%;
      min-height: 120px;
      background: #fff;
      border: 1px solid #F5E5EE;
      border-radius: 16px;
      padding: 16px 18px;
      font-family: 'DM Sans', sans-serif;
      font-size: 14px;
      color: #4A2030;
      resize: vertical;
      outline: none;
      transition: border-color 0.2s;
      line-height: 1.6;
    }}
    .notes-area:focus {{ border-color: var(--pink-light); }}
    .notes-area::placeholder {{ color: #D4B0C4; }}

    /* Footer */
    .footer {{
      text-align: center;
      padding: 40px 20px 20px;
      font-size: 13px;
      color: #D4A0B8;
    }}
    .footer-flowers {{ font-size: 18px; letter-spacing: 4px; margin-bottom: 6px; }}
  </style>
</head>
<body>

<!-- HERO -->
<div class="hero">
  <div class="clouds">
    <div class="cloud c1"></div>
    <div class="cloud c2"></div>
    <div class="cloud c3"></div>
  </div>
  <div class="petals">
    <div class="petal p1">🌸</div>
    <div class="petal p2">🌷</div>
    <div class="petal p3">🌸</div>
    <div class="petal p4">🌺</div>
    <div class="petal p5">🌸</div>
    <div class="petal p6">🌷</div>
    <div class="petal p7">🌸</div>
    <div class="petal p8">🌺</div>
  </div>
  <div class="juneteenth-line">✦ Juneteenth 2026 ✦</div>
  <div class="for-name">a day for {name_display}</div>
  <h1>{b['name']}</h1>
  <div class="hero-city">{b['city']}</div>
  <div class="vibe-chips">
    {''.join(f'<span class="vibe-chip">{v}</span>' for v in b["vibe"])}
    {fire_badge}
  </div>
</div>

<!-- BODY -->
<div class="body-wrap">

  <!-- Intention -->
  <div class="intention">
    <div class="intention-label">set the tone</div>
    {note_html}
    <div class="beach-blurb">{b['notes']}</div>
  </div>

  <!-- Meet up time -->
  <div class="meetup">
    <div class="meetup-icon">🌅</div>
    <div style="flex:1">
      <div class="meetup-label">leaving around</div>
      <input class="meetup-time" id="meetup" type="text" placeholder="fill in a time..."
             oninput="save('meetup', this.value)">
    </div>
  </div>

  <!-- Packing -->
  <div class="section">
    <div class="section-header">
      <span class="section-icon">🌺</span>
      <span class="section-title">bring this</span>
      <div class="section-line"></div>
    </div>
    <ul class="pack-list" id="pack-list">
      <li class="pack-item" data-id="sunscreen"   onclick="togglePack(this)"><span class="check-box"></span><span>sunscreen</span></li>
      <li class="pack-item" data-id="towels"      onclick="togglePack(this)"><span class="check-box"></span><span>towels</span></li>
      <li class="pack-item" data-id="water"       onclick="togglePack(this)"><span class="check-box"></span><span>water bottles</span></li>
      <li class="pack-item" data-id="snacks"      onclick="togglePack(this)"><span class="check-box"></span><span>snacks</span></li>
      <li class="pack-item" data-id="speaker"     onclick="togglePack(this)"><span class="check-box"></span><span>speaker</span></li>
      <li class="pack-item" data-id="blanket"     onclick="togglePack(this)"><span class="check-box"></span><span>blanket</span></li>
      <li class="pack-item" data-id="charger"     onclick="togglePack(this)"><span class="check-box"></span><span>portable charger</span></li>
      <li class="pack-item" data-id="cash"        onclick="togglePack(this)"><span class="check-box"></span><span>cash (for food)</span></li>
      <li class="pack-item" data-id="change"      onclick="togglePack(this)"><span class="check-box"></span><span>change of clothes</span></li>
      {fire_pack}
    </ul>
  </div>

  <!-- Food -->
  <div class="section">
    <div class="section-header">
      <span class="section-icon">🌷</span>
      <span class="section-title">where to eat — tap your pick</span>
      <div class="section-line"></div>
    </div>
    {food_cards}
  </div>

  <!-- Spots -->
  <div class="section">
    <div class="section-header">
      <span class="section-icon">🌸</span>
      <span class="section-title">while you're there — check off as you go</span>
      <div class="section-line"></div>
    </div>
    <ul class="pack-list">
      {spot_items}
    </ul>
  </div>

  <!-- Parking -->
  <div class="section">
    <div class="section-header">
      <span class="section-icon">✦</span>
      <span class="section-title">parking</span>
      <div class="section-line"></div>
    </div>
    <div class="parking-card">{b['parking']}<br><small style="color:#C0A0B0;margin-top:4px;display:block">~${b['budget_per_person']}/person total (food + parking)</small></div>
  </div>

  <!-- Notes -->
  <div class="section">
    <div class="section-header">
      <span class="section-icon">🌺</span>
      <span class="section-title">notes / thoughts</span>
      <div class="section-line"></div>
    </div>
    <textarea class="notes-area" id="notes" placeholder="write anything here..."
              oninput="save('notes', this.value)"></textarea>
  </div>

</div>

<div class="footer">
  <div class="footer-flowers">🌸 🌷 🌺</div>
  made with love &middot; juneteenth 2026
</div>

<script>
  const KEY = 'beach_plan_{beach_key}';

  function save(field, val) {{
    const data = JSON.parse(localStorage.getItem(KEY) || '{{}}');
    data[field] = val;
    localStorage.setItem(KEY, JSON.stringify(data));
  }}

  function load() {{
    const data = JSON.parse(localStorage.getItem(KEY) || '{{}}');

    // meetup time
    if (data.meetup) document.getElementById('meetup').value = data.meetup;

    // notes
    if (data.notes) document.getElementById('notes').value = data.notes;

    // packing
    (data.packed || []).forEach(id => {{
      const el = document.querySelector(`[data-id="${{id}}"]`);
      if (el) el.classList.add('checked');
    }});

    // spots
    (data.spots || []).forEach(id => {{
      const el = document.querySelector(`[data-id="${{id}}"]`);
      if (el) el.classList.add('done');
    }});

    // food pick
    if (data.food) {{
      const el = document.querySelector(`[data-id="${{data.food}}"]`);
      if (el) el.classList.add('picked');
    }}
  }}

  function togglePack(el) {{
    el.classList.toggle('checked');
    const checked = [...document.querySelectorAll('.pack-item.checked')].map(e => e.dataset.id);
    save('packed', checked);
  }}

  function toggleSpot(el) {{
    el.classList.toggle('done');
    const done = [...document.querySelectorAll('.spot-item.done')].map(e => e.dataset.id);
    save('spots', done);
  }}

  function pickFood(el) {{
    document.querySelectorAll('.food-card').forEach(c => c.classList.remove('picked'));
    el.classList.add('picked');
    save('food', el.dataset.id);
  }}

  load();
</script>
</body>
</html>"""
    return html


def main():
    args = sys.argv[1:]
    if not args:
        print("Usage: python3 make_plan.py <beach> [--for Name] [--note 'message']")
        print("       python3 make_plan.py dockweiler --for Kelli --note 'just trust me'")
        return

    beach_key = args[0].lower().replace("-", "_")
    if beach_key not in BEACHES:
        print(f"Unknown beach. Try: {', '.join(BEACHES.keys())}")
        return

    for_name = None
    note = None
    i = 1
    while i < len(args):
        if args[i] == "--for" and i + 1 < len(args):
            for_name = args[i + 1]; i += 2
        elif args[i] == "--note" and i + 1 < len(args):
            note = args[i + 1]; i += 2
        else:
            i += 1

    html = generate(beach_key, for_name, note)
    fname = f"plan_{beach_key}.html"
    with open(fname, "w") as f:
        f.write(html)

    print(f"Saved → {fname}")
    webbrowser.open(f"file://{os.path.abspath(fname)}")


if __name__ == "__main__":
    main()
